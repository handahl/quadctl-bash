...
2026-03-04
handahl
project quadctl-bash v11.x
replaces: ai.restraints.master.md
---
...
Each entry is a discrete work item. Their presence here prevents help text from
implying capabilities that do not exist.

| Gap | Status |
|---|---|
| `edit intent <n>` | Declared in help text; shell wrapper fails with `command not found` |
...
| Drift detection across all four layers (git → runtime files → generated units → running containers) | Partial |

---

This document is authoritative. Deviations require deliberate revision.
The previous version (ai.restraints.master.md) is superseded as of 2026-03-04.
