...
execute_matrix_view() {
    ...
    #log_info "Generating Quadlet Overview: ${filter_type^}"
    echo "quadctl"
    ...
    # 4. HEADER
    printf "%-17s %-8s %-10s %-10s %-8s %-10s %-12s %s\n" \
    ...
    for unit in $all_units; do
        ...
        if [[ "$p_health" == "inactive" ]]; then c_health="$Q_COLOR_GREY"; fi
        ...
        if [[ -n "$line_color" ]]; then
            line_output=$(printf "%b%-17s %-8s %-10s %-10s %-8s %-10s %-12s %s%b" \
                "$line_color" "$clean_name" "$d_val" "$s_active" "$display_sub" "$uptime_str" "$p_health" "$p_image" "$p_ports" "$Q_COLOR_RESET")
        else
            line_output=$(printf "%-17s %b%-8s%b %b%-10s%b %-10s %-8s %b%-10s%b %-12s %s" \
                "$clean_name" \
