...
get_containers_map() {
    local raw
    raw=$(query_podman_socket "/libpod/containers/json?all=true")
    ...
    # libpod endpoint: .Status = health state ("healthy"/"unhealthy"/"starting"/"")
    # Ports use snake_case: host_port / container_port / protocol
    echo "$raw" | jq -r '
        map(select(.Names and (length > 0)))
        | map({ (.Names[0] | ltrimstr("/")): . })
        | add // {}
    '
}
